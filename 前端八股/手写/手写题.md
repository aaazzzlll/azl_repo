...
```
Object.getPrototypeOf(obj)返回obj的原型（即内部 `[[Prototype]]` 属性的值）
...
3. 防抖:在事件被触发n秒后再执行回调函数，如果在这段时间内又触发该事件，则重新计时
使用场景：搜索框输入
```javascript
...
function myNew(fn,...args){
	...
	//如果构造函数返回了一个对象，就返回ret，否则返回obj(尊重构造函数的返回值)
...
```
[Object.setPrototypeOf(obj,prototype)](Object.setPrototypeOf(obj,prototype).md)
[Object.create(proto)](Object.create(proto).md)

除了Object.setPrototypeOf(Dog.prototype,Animal.prototype)还可以使用Dog.prototype=Object.create(Animal.prototype)
...
16. Promise.all()全部成功才成功，一个失败就失败
```javascript
...
```
通过 `forEach` 并行触发所有传入的 Promise（或普通值）执行，用 `Promise.resolve` 统一兼容 Promise / 普通值类型，按输入顺序存储每个 Promise 的成功结果，通过计数器判断所有 Promise 是否都成功（成功则调用 resolve 返回结果数组）；同时遵循快速失败规则，任意一个 Promise 失败时，通过 `catch(reject)` 隐式传递第一个失败原因给外层 reject 并立即终止，且所有逻辑均为并行执行（Promise 同时启动，总耗时由最慢的 Promise 决定）

Promise.resolve(value):
	value是普通值：`Promise.resolve()` 会创建新的已成功 Promise
	value是Promise实例：返回原实例

`catch(reject)` 是 `catch(err => reject(err))` 的简写，失败原因会自动透传给外层 reject，且 Promise 状态不可逆，因此只会保留第一个失败原因
...
18. 手写数组去重
```javascript
//简洁版：使用Set，但是如果数组中存在相同的对象或数组，就无法去重，因为两者引用地址不同
