- *使用 create-vue + Vite 初始化项目，自主搭建包含全局布局、动态导航菜单、通用组件的企业级前端模板，支持后续快速复用和多项目迁移。*
...
4. **动态导航菜单怎么实现的**
	我定义了全量菜单列表，通过computed和Pinia中的用户角色信息实时过滤掉key以/admin开头的菜单项，使非管理员用户看不到管理菜单

5. Vite配置
	 目前主要配置了**路径别名 `@`** 指向 `src` 目录，方便模块导入。同时保留了 `vite-plugin-vue-devtools` 插件用于开发调试