...
修改 router/index.ts 的路由配置：E2AG5YeWw8LgKsKYXs9Vrz+8AgTqaJUf5eaOZTC+BFg=
...
可以顺便给 loginUser 增加 TypeScript 类型，这样在后续开发时就会有字段提示：ChdipREN0jrjiXgJ/YkPo5ufCLwmKJhcbf8H+qMVE48=
...
先开发基础页面，可以按照需要增加一些前端校验，代码如下：E2AG5YeWw8LgKsKYXs9Vrz+8AgTqaJUf5eaOZTC+BFg=
...
编写表单提交后执行的函数，登陆成功后需要在全局状态中记录当前登录用户的信息，并跳转到主页：TJqV4WBBT2IFrp0o3vsnkggN7VzNkVQlxvcX5lXRIJI=
...
编写提交表单函数，可以增加一些前端校验，并且在注册成功后跳转到用户登录页：TJqV4WBBT2IFrp0o3vsnkggN7VzNkVQlxvcX5lXRIJI=
...
`<a-dropdown>` 组件有一个严格的规则：它的触发插槽（也就是默认插槽）里，必须只有一个根元素。所以必须使用`<a-space>`包裹头像和昵称
...
编写页面：上方搜索栏，下方表格，表格需要支持分页。B2z6448PQSc3P5d8RibCpEJZ9ZaX16ayHXniF5A52nY=

我们可以拆分为几个步骤来开发：

#### 1、表格

1）利用 [Ant Design 的表格组件](https://antdv.com/components/table-cn#components-table-demo-basic)，找到需要的组件进行复制，先展示全部用户信息。uPqbd3DiPd3iSPyF1TJ20nEUpYukqZektmm49mRYrTY=
...
定义表格列：title是显示在前端页面的文字，dataIndex是从后端取过来字段的名称
...
4）对于图片、用户角色、创建时间、更新时间之类的数据，我们可以有更好的展示方式。9JcB9KvGktsJeCekY//R96Nlv+uCpT8TeUU7sYgObzE=
...
效果如图：iIeB8mD2J1oXqiOtY3taWTPWsqKxcGCkYX8MdOVhu+A=

![](https://pic.code-nav.cn/course_picture/1612112932991139842/Wuoms7ujliXOYmEA.webp)

#### 2、分页

1）表格组件 [支持分页](https://antdv.com/components/table-cn#pagination)，首先编写一个分页变量，指定当前页号、页面大小、数据总数、展示总数的文案等参数：TJqV4WBBT2IFrp0o3vsnkggN7VzNkVQlxvcX5lXRIJI=
...
2）编写 doTableChange 函数，当用户切换页号和页面大小时，需要更新 searchParams 搜索条件的值，并触发搜索：sV0XXF40kQah+NcVxFP1y4nQNGtV/XyQvOt/FJKWUoA=
...
2）点击搜索按钮时会触发表单提交，表单提交时需要将页号重置为 1，并获取数据。
**如果不重置页码：**
- 你的请求参数是：`searchParams = { userName: '张三丰', current: 10 }`。
- 后端逻辑：去查叫 "张三丰" 的人，查出来总共 3 条。然后尝试给你取 **第 10 页** 的数据（即第 91-100 条）。
- **结果**：后端返回空数组 `[]`（因为总共才 3 条，哪里来的第 10 页？）。
- **用户体验**：用户明明搜了名字，页面却显示“暂无数据”，这会造成严重的 Bug 错觉。
...
graph TD
    `A用户输入账号/用户名] -->|v-model 自动同步| 
    `B(searchParams 数据更新)`
    `C[用户点击搜索按钮] -->|@finish| D{doSearch 函数}`
    `D -->|重置 current=1| E(searchParams 修正)`
    `E -->|调用| F{fetchData 函数}`
    `F -->|发送 POST 请求| G[后端 API]`
    `G -->|返回数据 res| F`
    `F -->|dataList.value = res| H[a-table 表格重新渲染]`
    `F -->|total.value = res| I[分页组件重新渲染]`
#### 4、操作
...
然后给删除按钮绑定事件：uPqbd3DiPd3iSPyF1TJ20nEUpYukqZektmm49mRYrTY=
...
虽然后端获取用户列表的接口做了权限校验，防止非管理员用户获取到。但一般情况下，为了保证系统安全和提升用户体验，前端也需要对权限进行控制。9JcB9KvGktsJeCekY//R96Nlv+uCpT8TeUU7sYgObzE=

有 2 种实现方式：单页面控制权限，或者全局控制权限。

思路都是一致的，在进入某个页面时判断用户是否具有该页面的权限，无非是把权限校验相关的代码写在单个页面内，还是写到一个独立的文件中罢了。

\*\*建议编写独立的全局权限控制文件。\*\*可以利用 Vue Router 的路由守卫实现，每次切换并进入页面前，都会检查一下当前用户是否具有特定页面的权限。B2z6448PQSc3P5d8RibCpEJZ9ZaX16ayHXniF5A52nY=
...
注意，上述代码中，我们为了确保页面刷新时，从后端拿到用户信息后再进行权限校验，使用 await 等待后端接口返回，并重新赋值给 loginUser。同时，为了防止每次切换路由都从远程获取用户信息，定义了 firstFetchLoginUser 变量，用于控制在刷新页面后只会请求后端一次。xo0dnOlnBf7Ws27/qGm2TPReUJyIxD/DuwJOjlvKUe0=
...
虽然已经满足了需求，但是如果用户不是管理员，其实也不应该看到 “用户管理” 菜单。因此我们还要修改 GlobalHeader 菜单项配置，根据权限决定是否展示某些菜单项。9JcB9KvGktsJeCekY//R96Nlv+uCpT8TeUU7sYgObzE=
...
测试效果，未登录时不会看到 “用户管理” 菜单：TJqV4WBBT2IFrp0o3vsnkggN7VzNkVQlxvcX5lXRIJI=
...
实现方案：9JcB9KvGktsJeCekY//R96Nlv+uCpT8TeUU7sYgObzE=
...
新建 access 目录，所有权限管理相关的代码都放在该目录下，模块化。只要不引入，就不会生效。E2AG5YeWw8LgKsKYXs9Vrz+8AgTqaJUf5eaOZTC+BFg=
...
2）修改路由配置文件，在 meta 中补充需要的权限：B2z6448PQSc3P5d8RibCpEJZ9ZaX16ayHXniF5A52nY=
...
Q：为什么？sV0XXF40kQah+NcVxFP1y4nQNGtV/XyQvOt/FJKWUoA=
...
注意，必须保证 pinia 初始化在这段代码执行前，所以需要将 `useLoginUserStore()` 函数放到 `router.beforeEach` 参数里。TJqV4WBBT2IFrp0o3vsnkggN7VzNkVQlxvcX5lXRIJI=

参考：[https://pinia.vuejs.org/core-concepts/outside-component-usage.html](https://pinia.vuejs.org/core-concepts/outside-component-usage.html)
...
默认的 loginUser 是没有 userRole 的，只要获取过，哪怕未登录，也可以给设置一个 userRole 为 "notLogin"。uPqbd3DiPd3iSPyF1TJ20nEUpYukqZektmm49mRYrTY=
...
之后记得移除 App.vue 中获取登录信息的调用。B2z6448PQSc3P5d8RibCpEJZ9ZaX16ayHXniF5A52nY=

6）根据权限控制菜单显隐。

需求：只有具有权限的菜单，才对用户可见

原理：xo0dnOlnBf7Ws27/qGm2TPReUJyIxD/DuwJOjlvKUe0=
