...
2. 排序链表
采用归并排序的分治思想，使用快慢指针找到中点，递归将链表拆分为单个节点，再通过伪头结点将有序子链表逐层合并，最终得到完整的有序链表。递归终止条件：空链表或单个节点(拆分的区间是左开右闭)
```javascript
    if(head===null){
        return head
    }
...
# 图
1. 岛屿数量
双层循环遍历网格，发现未沉没的陆地就islandCount+1，在
...
4. 每日温度
利用单调递减栈存储未找到更高温度的日期下标，遍历温度数组时，若栈非空且当前温度高于栈顶下标对应的温度，就弹出栈顶并计算两者下标差作为结果，否则将当前下标压入栈，最终未弹出的下标结果保持默认 0
...
1. 数组中的第K个最大元素
分桶式快速选择：
通过随机选择基准值将数组划分为大于、等于、小于基准值的三个桶，利用分治思想，仅递归处理包含第 k 大元素的桶以缩小查找范围，最终在 equal 桶中得到并返回结果
...
2. 前K个高频元素
基于桶排序求解，首先构建每个数字和对应个数的映射，接着定义一个bucket数组，桶的下标 = 元素出现频率，桶的值 = 对应频率的所有元素（此处使用map.entries()得到map中的`[key,value]`），最后逆序遍历桶，将前k个数字放到res中，循环的时候跳过没有定义过的桶，返回res.slice(0,k)
