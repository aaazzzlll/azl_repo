...
注意：最后要返回dummy.next，这样才能解决只有一个节点和删除头节点的边界情况
$$
时间复杂度：O(L)空间复杂度：O(1)
$$

58. 打乱数组
该算法通过构造函数保存数组原始值和当前值，reset 方法还原数组为原始副本，shuffle 方法采用 Fisher-Yates 洗牌算法（从后往前遍历数组，每轮随机生成 0 到当前索引的随机位置，交换当前索引与随机位置的元素），实现数组的随机打乱且可重置回初始状态。
*reset 时用`slice()`创建原始数组副本，避免引用导致原始值被修改。`slice()是浅拷贝`*
...
60. 二叉树的后序遍历
递归：思路同上
迭代：借助栈结构，先将根节点入栈，循环弹出栈顶节点并存储值，按 “先左后右” 的顺序将子节点入栈（得到根→右→左的遍历结果），最后反转结果数组，实现左→右→根的后序遍历。
$$
时间复杂度：O(n)空间复杂度：O(n)
$$

61. 最小路径和
该动态规划算法通过定义`dp[i][j]`为起点 (0,0) 到 (i,j) 的最小路径和，先初始化第一行 / 列的累加路径和，再按 “取上方 / 左方最小路径和加当前格子值” 的规则填充 dp 数组，最终`dp[row-1][column-1]`即为终点的最小路径和
$$
时间复杂度：O(m*n)空间复杂度：O(m*n)
$$
